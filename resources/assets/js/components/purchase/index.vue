            ...
            <div class="card mb-3 shadow no-b r-0">
                ...
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <tr v-for="(item, index) in purchases.data">
                            <td>{{ index + 1 }}</td>
                            <td><a :href="purchaseHrefLink + item.purchase_no + '/show'">{{ item.invoice_no }}</a></td>
                            <td><a :href="purchaseHrefLink + item.purchase_no + '/show'">{{ item.purchase_no }}</a></td>
                            <td><a :href="supplierHrefLink + item.supplier.code + '/details'">{{ item.supplier.name }}
                            </a></td>
        ...
        methods : {
            ...
            deleteItem(index, id) {
                this.$swal({
                    ...
                    if (result.value) {
                        this.$axios.delete('purchase/' + id, this.$auth.getHeader()).then(response => {
                            this.$store.dispatch('setPurchases', '');
                            this.$swal(
                                'Deleted!',
