<template>
    <div>
        ...
        <div v-if="typeof customers.data != 'undefined' && customers.data.length > 0">
            <div class="card mb-3 shadow no-b r-0">
                ...
                <div class="box-body no-padding">
                    <table class="table table-striped">
                        ...
                        <tr v-for="(item, index) in customers.data">
                            <td>{{ index + 1 }}</td>
                            ...
                            <td>{{ item.balance }}</td>
                            <td>
                                <a href="#modal" @click="$store.dispatch('setCustomerEditData', item)">
                                    <i class="icon-edit"></i>
                                </a>
                                &nbsp;
    ...
    export default{
        data () {
            return {
                ...
                page : 1,
                pageCount : 2,
                url : process.env.MIX_APP_URL + 'customer/'
            }
        },
        components : {
            edit,
        ...
        methods : {
            ...
            deleteItem(index, id) {
                this.$swal({
                    ...
                    if (result.value) {
                        this.$axios.delete('customer/' + id, this.$auth.getHeader()).then(response => {
                            this.$store.dispatch('setCustomers', '');
                            this.$swal(
                                'Deleted!',
