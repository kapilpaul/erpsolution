        ...
        <div class="form-group form-float">
          <div class="form-line">
            ...
            <select class="form-control custom-select select2" id="bank_id">
              <option v-for="(item, index) in banks" :value="index">{{
                item
              }}</option>
            </select>
          </div>
...
export default {
  ...
  methods: {
    add() {
      ...
      this.itemData.account_type = document.getElementById(
        "account_type"
      ).value;
      this.itemData.bank_id = document.getElementById("bank_id").value;

      this.$axios
        .post("banks/transaction", this.itemData, this.$auth.getHeader())
        .then(response => {
