                ...
                <div class="box-body no-padding">
                    <table class="table table-striped">
                        ...
                        <tr v-for="(item, index) in fliteredItems">
                            <td>{{ index + 1 }}</td>
                            <td><a :href="bankDetailsUrl + item.code + '/show'">{{ item.name }}</a></td>
                            <td>{{ item.ac_name }}</td>
                            <td>{{ item.ac_no }}</td>
                            <td>{{ item.branch }}</td>
                            <td>{{ item.balance }}</td>
                            <td>
                                <a href="#modal" @click="$store.dispatch('setBankEditData', item)">
                                    <i class="icon-edit"></i>
                                </a>
                                &nbsp;
        ...
        methods : {
            ...
            deleteItem(index, id) {
                this.$swal({
                    ...
                    if (result.value) {
                        this.$axios.delete('banks/' + id, this.$auth.getHeader()).then(response => {
                            this.$swal(
                                'Deleted!',
