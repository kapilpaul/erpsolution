            ...
            <div class="card mb-3 shadow no-b r-0">
                ...
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <tr v-for="(item, index) in invoices.data">
                            <td>{{ index + 1 }}</td>
                            <td><a :href="invoiceHrefLink + item.invoice_no + '/show'">{{ item.invoice_no }}</a></td>
                            <td v-if="typeof item.customer != 'undefined'">
                                <a :href="customerHrefLink + item.customer.code + '/details'">{{ item.customer.name }}
                                </a>
                            </td>
                            <td v-else>
                                <a :href="customerHrefLink + item.customer_code + '/details'">{{ item.customer_name }}
                                </a>
                            </td>
                            <td>{{ item.date }}</td>
    ...
    export default{
        data () {
            return {
                ...
                page : 1,
                pageCount : 2,
                invoiceHrefLink : process.env.MIX_APP_URL + 'invoice/',
                customerHrefLink : process.env.MIX_APP_URL + 'customer/'
            }
        },
        components : {
            paginate,
        ...
        methods : {
            ...
            deleteItem(index, id) {
                this.$swal({
                    ...
                    if (result.value) {
                        this.$axios.delete('invoice/' + id, this.$auth.getHeader()).then(response => {
                            this.$store.dispatch('setInvoices', '');
                            this.$swal(
                                'Deleted!',
