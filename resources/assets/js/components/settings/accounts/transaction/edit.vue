...
export default {
  ...
  computed: {
    ...
    slip_id: {
      ...
      set(value) {
        this.$store.dispatch(
          "setBankTransactionEditData",
          this.updateValue("slip_id", value)
        );
      }
    },
    description: {
  ...
  methods: {
    ...
    update() {
      ...
      this.$axios
        .patch("banks/transaction/" + id, data, this.$auth.getHeader())
        .then(response => {
          if (this.code) {
