          ...
          <table class="table table-hover ">
            <tbody>
              <tr v-for="(item, index) in products.data" :key="item.id">
                ...
                <td>
                  <a
                    class="btn-fab btn-fab-sm btn-primary shadow text-white"
                    href="#modal"
                    @click="$store.dispatch('setProductsEditData', item)"
                  >
                  ...
                  <a
                    ...
                    href="#"
                    @click.prevent="deleteItem(index, item.id)"
                  >
...
export default {
  ...
  data() {
    return {
      ...
      page: 1,
      pageCount: 2,
      productUrl: process.env.MIX_APP_ROOT + "/products/"
    };
  },
  components: {
    edit,
  ...
  methods: {
    ...
    deleteItem(index, id) {
      this.$swal({
        ...
        if (result.value) {
          this.$axios
            .delete("products/" + id, this.$auth.getHeader())
            .then(response => {
              this.$store.dispatch("setProducts", "");
