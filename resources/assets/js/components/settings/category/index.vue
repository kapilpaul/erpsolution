<template>
    <div>
                ...
                <div class="box-body no-padding">
                    <table class="table table-striped">
                        ...
                        <tr v-for="(item, index) in fliteredItems">
                            ...
                            <td>
                                <a href="#modal" @click="$store.dispatch('setCategoryEditData', {'id': item.id, 'name' :
                        item.name})">
                                    <i class="icon-edit"></i>
                                </a>
                                &nbsp;
        ...
        <no-data :name="'Category'" :url="url" v-else></no-data>
    </div>
</template>
    ...
    export default{
        data () {
            return {
                items : [],
                search : "",
                url : process.env.MIX_APP_URL + 'category/'
            }
        },
        components : {
            edit,
        ...
        methods : {
            ...
            deleteItem(index, id) {
                this.$swal({
                    ...
                    if (result.value) {
                        this.$axios.delete('category/' + id, this.$auth.getHeader()).then(response => {
                            this.getItems();
                            this.$swal(
                                'Deleted!',
